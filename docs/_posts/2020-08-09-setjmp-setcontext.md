...
可以参考 [musl-libc 的实现][4]。可以说 ``setjmp()`` 和 ``longjmp()`` 是相当简练的。只提供了两个功能，一个是记录当前的位置，另一个是跳转到指定的位置。
...
``Boost::context`` 用汇编实现了平台相关的 ``fcontext_t`` ，它的性能据说比 ``ucontext_`` [高一到两个数量级][7] 。``fcontext_`` [保存的上下文][8] 有
